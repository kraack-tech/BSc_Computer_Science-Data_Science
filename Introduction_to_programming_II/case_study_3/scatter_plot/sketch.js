...
//we load data in preload (this guarentees that data is ready is setting up the site!)
function preload(){
...
function setup() {
	...
	//for scatterplot plot: (quiz and project columns - see if they corrolate)
	...
	/*draw scatterplot axis (x & y from 0 to 100 to store all possible student marks ([0, 100, 0, 100],) 
	and then we make labels every 10th pixel (0, 100, 0, 100, [10, 10])) */
	myScatter.setupAxis(0, 100, 0, 100, 10, 10)

	//choose columns and the size and color of the ellipse (ie the dot point)
	myScatter.addDataPoints(quizMarks, projectMarks, 3, 'blue')

	//add line of best fit (from stat 2 y=m+x+b) 
	//remember to convert string to number!! in 'this.lineOfBestFit' with parse to float 'parseFloat(xValues[i])'
	myScatter.lineOfBestFit(quizMarks, projectMarks, 'red')

	/* 	//exploring data in console
	var firstStudent = studentData.getRow(0);
	var firstXander = studentData.findRow('Xander', 'firstName');
	var allXanders = studentData.findRows('Xander', 'firstName');
	//print to console
	console.log(firstStudent);
	console.log(firstXander);
	console.log(allXanders);
	//get actual values
	...
	this.lineOfBestFit = function(xValues, yValues, colour) {
		...
		if (xValues.length == yValues.length) {
			...
			for (var i = 0; i < xValues.length; i++) {
				...
				sumXY += parseFloat(xValues[i] * yValues[i]);
				sumX2 += parseFloat(xValues[i] * xValues[i]);
			}
